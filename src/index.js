...
async function main() {
  ...
  if (process.argv.length < 3) {
    ...
    console.error(
      "Example: npm start https://www.udemy.com/course/your-course-name",
    );
    process.exit(1);
  }
  ...
  const downloadSrt = await new Promise((resolve) => {
    rl.question(
      "Do you want to download transcripts as .srt files with timestamps as well? (yes/no) [no]: ",
      (answer) => {
  ...
  let tabCount = await new Promise((resolve) => {
    rl.question(
      `How many tabs do you want to use for downloading transcripts? (default is 5) [5]: `,
      (answer) => {
        const normalized = answer.trim();
        const value = normalized ? parseInt(normalized, 10) : 5;
        resolve(value);
      },
    );
  });

  if (Number.isNaN(tabCount) || tabCount < 1) {
    tabCount = 5;
  }
  if (tabCount > 20) {
    tabCount = 20; // avoid too many tabs
  }
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2 && !loginPageLoaded; attempt++) {
      try {
        await page.goto("https://www.udemy.com/join/passwordless-auth", {
    ...
    if (!process.env.UDEMY_EMAIL) {
      console.error(
        "UDEMY_EMAIL not found in .env file. Please configure your credentials.",
      );
      process.exit(1);
    }
    ...
    await page.type('input[name="email"]', process.env.UDEMY_EMAIL, {
      delay: 100,
    });
    ...
    await page.$eval(
      '[data-purpose="code-generation-form"] [type="submit"]',
    ...
    console.log(
      "You have 5 minutes to enter the verification code before the program times out.",
    );
    ...
    await page.waitForSelector('[data-purpose="otp-text-area"] input', {
      timeout: 60000,
    });
    await page.type('[data-purpose="otp-text-area"] input', verificationCode, {
      delay: 100,
    });

    // Submit the verification form
    await page.$eval(
      '[data-purpose="otp-verification-form"] [type="submit"]',
      (element) => element.click(),
    );
    ...
    const courseId = await getCourseId(page, courseUrl);

    if (!courseId) {
      throw new Error(
        "Could not retrieve course ID. Make sure you are logged in and the course URL is correct.",
      );
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        const responseJson = await page.evaluate(async (url) => {
        ...
        console.warn(
          `[Attempt ${attempt}] Failed to fetch course content: ${err.message}`,
        );
        if (attempt < maxAttempts) {
          ...
          throw new Error(
            "Could not retrieve course content. Make sure you have access to this course and try again.",
          );
        }
      }
    }
...
function processCourseStructure(results) {
  ...
  // Sort results by sort_order (ascending order to follow natural course flow)
...
// Get course ID from page context or fallback
async function getCourseId(page, courseUrl) {
  let courseId = await page.evaluate(() => {
    const body = document.querySelector("body");
    if (body) {
      if (body.dataset.clpCourseId) return body.dataset.clpCourseId;
      if (body.dataset.courseId) return body.dataset.courseId;
    }

    if (
      window.__INITIAL_DATA__ &&
      window.__INITIAL_DATA__.course &&
      window.__INITIAL_DATA__.course.id
    ) {
      return window.__INITIAL_DATA__.course.id;
    }

    if (window.__udemy && window.__udemy.course && window.__udemy.course.id) {
      return window.__udemy.course.id;
    }
...
async function downloadTranscripts(
  ...
  await Promise.all(
    chunks.map(async (chunk, tabIndex) => {
      const page = await browser.newPage();
...
async function processLecture(
  ...
  try {
    ...
    await page
      ...
      .catch(() => {
        console.log(
          `Note: Video player not fully loaded for lecture: ${lecture.title}, but continuing anyway`,
        );
      });
      ...
      try {
        ...
        if (buttonExists) {
          ...
          if (isPanelVisible) {
            ...
            console.log(
              "Button clicked but panel did not appear, trying next selector",
            );
          }
        }
      } catch (error) {
    ...
    if (!transcriptButtonFound) {
      console.log(
        `No transcript button found/clicked successfully for lecture: ${lecture.title}. This lecture might not have a transcript.`,
      );
      // Create a placeholder file
      fs.writeFileSync(
        path.join(__dirname, "../output", `${sanitizedFilename}.txt`),
    ...
    if (!transcriptText || transcriptText.trim() === "") {
      console.log(
        `No transcript content available for lecture: ${lecture.title}`,
      );
      return;
    }
      ...
      for (const caption of lecture.captions) {
        try {
          ...
          const srtPath = path.join(
            __dirname,
            "../output",
            `${sanitizedFilename} [${langTag}].srt`,
          );
          fs.writeFileSync(srtPath, srtContent, "utf8");
          console.log(`SRT saved: ${sanitizedFilename} [${langTag}]`);
        } catch (err) {
          console.log(
            `Error downloading caption [${caption.locale_id}] for ${sanitizedFilename}: ${err.message}`,
          );
        }
      }
    } else if (downloadSrt) {
